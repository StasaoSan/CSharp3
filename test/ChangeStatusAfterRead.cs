...
public class ChangeStatusAfterRead
{
    ...
    public void ChangeStatusAfterReadTest(Message message)
    {
        var user = new User("John", "Doe", new Logger());
        var userDestination = new DestinationUser(user);
        userDestination.SendMessage(message);
        userDestination.User.ReadMessage(message.Id);
        Assert.True(user.IsMessageRead(message.Id));
    }
}